...
describe("Book", () => {
  it("should create a Book instance with valid data", () => {
    const book = new Book("Title", "Author", "123-456-789X", "A book description");

    expect(book.title).toBe("Title");
    expect(book.author).toBe("Author");
    expect(book.isbn).toBe("123-456-789X");
    expect(book.description).toBe("A book description");
    expect(book.stars).toBe(0);
  });

  it("should throw an error if any field is empty", () => {
    expect(() => new Book("", "Author", "123-456-789X", "Description")).toThrow(
      "Please fill in all fields"
    );
    expect(() => new Book("Title", "", "123-456-789X", "Description")).toThrow(
      "Please fill in all fields"
    );
    expect(() => new Book("Title", "Author", "", "Description")).toThrow(
      "Please fill in all fields"
    );
    expect(() => new Book("Title", "Author", "123-456-789X", "")).toThrow(
      "Please fill in all fields"
    );
  });
  ...
  it("should not throw an error if ISBN is valid", () => {
    expect(() => new Book("Title", "Author", "123-456-789X", "Description")).not.toThrow();
  });
});
